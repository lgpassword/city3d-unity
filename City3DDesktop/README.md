...
- **WPF前端**：负责UI交互、数据管理、API调用
- **Unity渲染后端**：负责3D场景渲染和可视化
- **进程间通信**：使用命名管道（Named Pipes）传输场景数据

```
┌─────────────────────────────────────┐
│   WPF Desktop Application (主进程)   │
│  ┌─────────────────────────────┐   │
│  │  UI Layer (XAML)            │   │
│  ├─────────────────────────────┤   │
│  │  Business Logic (C#)        │   │
...
1. **用户输入** → WPF UI
2. **API调用** → AI识别、OSM数据、海拔数据
3. **数据处理** → WPF Services层
4. **场景数据** → 通过命名管道发送到Unity
5. **3D渲染** → Unity接收并渲染场景
6. **窗口嵌入** → Unity窗口嵌入到WPF容器
...
1. **构建Unity渲染引擎**
   ...
   WPF应用需要Unity渲染引擎来显示3D场景。首次运行前需要构建Unity项目：
   ...
   **方法二：手动构建**
   - 在Unity中打开主项目（D:\github\3d）
   - 选择 File > Build Settings
   - 平台选择 Windows (x86_64)
   - 构建到: `City3DDesktop/City3DDesktop/bin/Debug/net8.0-windows/UnityRenderer/`
   - 文件名: `City3D.exe`

   详细步骤见 [UNITY_BUILD_GUIDE.md](../UNITY_BUILD_GUIDE.md)
